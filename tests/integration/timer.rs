...
#[test]
fn test_status_reflects_config_without_start() -> Result<(), Box<dyn std::error::Error>> {
    use std::io::Write;

    // Create a temporary config file with custom durations
...
"#
    ...
    // Check status WITHOUT calling start command first
    let status = daemon.get_status()?;
    let text = status.get("text").and_then(|v| v.as_str()).unwrap();

    // Should show 50:00 from config, not 25:00 hardcoded default
    assert!(
        text.contains("50:00"),
        "Status should reflect config work duration (50 min) without calling start. Got: {}",
        text
    );
