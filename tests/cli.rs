...
mod integration {
    ...
    impl TestDaemon {
        ...
        fn send_command(&self, args: &[&str]) -> Result<Value, Box<dyn std::error::Error>> {
            ...
            let output = Command::new(&binary_path)
                ...
                .output()
                .map_err(|e| {
                    format!("Failed to run command with binary '{}': {}", binary_path, e)
                })?;
                    ...
                    if text.contains("00:00") && !timer_completed {
                        ...
                        if let Some(text_after) = status_after.get("text").and_then(|v| v.as_str())
                        {
                            ...
                            if text_after.contains("00:00") {
                                ...
                                println!(
                                    "Auto-transition didn't occur, manually triggering with skip"
                                );
    ...
    fn test_status_default_format_returns_json() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Get status with default format (should be waybar)
        let status = daemon.get_status()?;

        // Should be JSON object with expected fields
        assert!(
            status.is_object(),
    ...
    fn test_status_waybar_format_returns_json() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Get status with explicit waybar output
